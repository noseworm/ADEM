...
class DialogEncoder(EncoderDecoderBase):
    ...
    def GRU_dialogue_step(self, h_t, m_t, bnmask_t, hs_tm1, *args):
        ...
        hs_tilde = self.sent_rec_activation(hs_tilde_normop_h_inp + hs_tilde_normop_hs_inp + self.bs_hh)

        #hs_tilde = self.dialogue_rec_activation(T.dot(h_t, self.Ws_in) + T.dot(rs_t * hs_tm1, self.Ws_hh) + self.bs_hh)
...
class UtteranceDecoder(EncoderDecoderBase):
    ...
    def build_decoder(self, decoder_inp, x, xmask=None, xdropmask=None, y=None, y_neg=None, mode=EVALUATION, prev_state=None, step_num=None):
        ...
        if mode == UtteranceDecoder.EVALUATION:
            ...
            return target_probs, hd, outputs
...
class DialogEncoderDecoder(Model):
    ...
    def build_eval_function(self):
        if not hasattr(self, 'eval_fn'):
            ...
            if self.add_latent_gaussian_per_utterance:
                ...
                self.eval_fn = theano.function(inputs=[self.x_data, self.x_data_reversed, self.x_max_length, self.x_cost_mask, self.x_reset_mask, self.ran_gaussian_cost_utterance, self.ran_uniform_cost_utterance, self.x_dropmask], 
                                                outputs=[self.evaluation_cost, self.softmax_cost], 
                                                updates=self.state_updates,
                                                on_unused_input='warn', name="eval_fn")
    ...
    def build_next_probs_function(self):
        if not hasattr(self, 'next_probs_fn'):
            ...
            if self.add_latent_gaussian_per_utterance:
                ...
                if self.add_latent_gaussian_per_utterance:
                    #_gaussian_prior_out, _ = self.latent_gaussian_utterance_variable_prior_encoder.build_encoder(hs_to_condition_latent_variable_on, self.beam_x_data)
    ...
    def __init__(self, state):
        ...
        if self.add_latent_gaussian_per_utterance:
            ...
            self.latent_gaussian_utterance_variable_prior_encoder = DialogLevelLatentGaussianEncoder(self.state, self.sdim, self.latent_gaussian_per_utterance_dim, self.rng, self, 'latent_gaussian_utterance_prior')
            ...
            self.latent_gaussian_utterance_variable_approx_posterior_encoder = DialogLevelLatentGaussianEncoder(self.state, posterior_input_size, self.latent_gaussian_per_utterance_dim, self.rng, self, 'latent_gaussian_utterance_approx_posterior')
            ...
            self.gaussian_posterior_mean_combination = LinearCombination(self.state, posterior_input_size, self.latent_gaussian_per_utterance_dim, False, 0.0, 0.0, self.rng, self, 'latent_gaussian_utterance_approx_posterior_mean_combination')
            ...
            # This allows model to revert back to prior easily for dimensions, where it is uncertain.
            self.posterior_variance_combination = LinearCombination(self.state, posterior_input_size, self.latent_gaussian_per_utterance_dim, True, self.min_latent_gaussian_variable_variances, self.max_latent_gaussian_variable_variances, self.rng, self, 'latent_gaussian_utterance_approx_posterior_variance_combination')
        ...
        target_probs, self.hd, target_probs_full_matrix = self.utterance_decoder.build_decoder(self.hd_input, training_x, xmask=training_hs_mask, xdropmask=training_x_dropmask, y=training_y, mode=UtteranceDecoder.EVALUATION, prev_state=self.phd)
        ...
        # Add normalization operator updates, which projects gamma parameters back to their constrained intervals:
