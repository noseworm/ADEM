...
def train(train_x, val_x, test_x, train_y, val_y, test_y, train_l, val_l, test_l, \
    ...
    model2 = LinearEvalModel(input=x, feat=feat, emb_dim=emb_dim, batch_size=batch_size, init_mean=init_mean, init_range=init_range, \
            mean_length=mean_length, feat_dim=feat_dim)
    ...
    emb_cost =  emb_len_loss_const * model.hinge_loss_len(l) + emb_l1_reg * model.l1_regularization_F_I() + emb_loss_const * model.squared_error(y)
...
if __name__ == '__main__':
    ...
    def shuffle_dataset(contexts, gts, models, scores, is_list=True):
        if not os.path.exists(short_shuffle_file):        
            shuffled_order = range(len(contexts))
            random.shuffle(shuffled_order)
            long_shuffled_order = [4*i for i in shuffled_order]
            lso = []
            for i in xrange(len(long_shuffled_order)):
    ...
    if shuffle:
        twitter_contexts, twitter_gtresponses, twitter_modelresponses, twitter_human_scores = shuffle_dataset(twitter_contexts, \
    ...
    twitter_dialogue_embeddings, nuisance_features, aux_features = preprocess_data(twitter_contexts, twitter_gtresponses, twitter_modelresponses, context_embedding_file, \
    ...
    if test_liu_data:
        ...
        twitter_dialogue_embeddings_liu, _, liu_feat = preprocess_data(twitter_contexts_liu, twitter_gtresponses_liu, twitter_modelresponses_liu, \
        ...
        for lr in lr_list:
            for l2reg in l2reg_list:
                for l1reg in l1reg_list:
                    ...
                    summary, folder_name, best_params, model = train(train_x, val_x, test_x, train_y, val_y, test_y, train_nuis, val_nuis, test_nuis, \
                            init_mean, init_range, learning_rate=lr, l2reg=l2reg, l1reg=l1reg, \
